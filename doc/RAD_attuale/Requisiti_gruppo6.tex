...
\section{TODO: BOZZA SCOPE CHAT DA INSERIRE IN DOMINIO DEL PROBLEMA - DA RIVEDERE LE CONVENZIONI}
In aggiunta a quanto detto in precedenza, è prevista anche l'implementazione di una nuova funzionalità nell’App UNIMOL che prevede la chat sia tra soli \textit{studenti} che tra \textit{studenti} e \textit{docenti}.
L'idea di base è quella di fornire un sistema di comunicazione semplice ed affidabile, che eviti la condivisione di contatti personali come numero di telefono ed email.
Le chat saranno create automaticamente in base agli alle varie coorti e ai corsi. La chat verrà abilitata per ciascun utente nel momento in cui egli farà il login nell’App Studenti.
Le chat saranno composte da canali utili per dividere i \textit{topic} e per ogni chat è previsto un canale di default che sarà anche quello principale.
Le chat verranno resettate di anno in anno, lasciando però attive quelle relative agli esami ancora non sostenuti.\newline
La chat si compone di 4 componenti principali:
\begin{enumerate}
\item Chat su App Studenti:\newline
Ogni studente avrà a disposizione all’interno dell’App Studenti UNIMOL una chat relativa all’anno del corso di studio, oltre che ad una chat per ogni singolo corso facente parte del piano di studi.
Lo \textit{Studente} non ptrà condividere allegati.

\item Chat su App Docenti:\newline
Nell’App docenti, ogni \textit{docente} sarà amministratore di una chat riferita ad ogni suo singolo corso, in cui ci saranno gli studenti che dovranno sostenere tale esame durante l'anno accademico corrente. La chat verrà creata automaticamente ma disabilitata di \textit{default}, se il \textit{docente} vorrà utilizzarla dovrà prima provvedere all'abilitazione.\newline
Il \textit{docente} potrà anche creare canali nelle sue chat, gestire i membri e segnalare eventuali messaggi inappropriati e inviare allegati.

\item Pannello di amministrazione:\newline
Il pannello di amministrazione sarà concepito per gestire le chat e gli utenti, supportare i docenti e gestire i messaggi inopportuni. Principalmente dal pannello si potranno attivare e disattivare chat, gestire i canali di cui si compongono le chat, aggiungere, rimuovere e silenziare gli \textit{studenti} dai canali, e infine revisionare i messaggi segnalati.
...
Il server sarà in grado di filtrare le parole offensive e inappropiate prensenti nei messaggi, fornire un supporto a chat testuale e alla condivisione di allegati.
\end{enumerate}
